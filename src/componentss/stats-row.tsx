import { CalendarCheck2, CalendarDays, UserCheck, TrendingUp } from "lucide-react";
...
const stats = [
  ...
  },
  {
    label: "Patients Seen",
    value: "21",
    sub: "This week across all doctors",
    icon: UserCheck,
      ...
      {stats.map((stat) => (
        <Card key={stat.label} className="border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-5 flex items-start gap-4">
            <div className={`h-10 w-10 shrink-0 rounded-lg ${stat.bg} ${stat.border} border flex items-center justify-center ${stat.color}`}>
